...
void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent,
  ...
  file_name =
      std::string(filePath.substr(0, found)) + "/../scan_mode/" + file_name;
...
void LivoxPointsPlugin::SendRosTf(const ignition::math::Pose3d &pose,
  ...
  tfBroadcaster->sendTransform(tf::StampedTransform(
      tf, ros::Time::now(), raySensor->ParentName(), raySensor->Name()));
}
...
#pragma omp parallel for
  for (int i = 0; i < points_pair.size(); ++i) {
    ...
    if (verticle_index < verticalRayCount && horizon_index < rayCount) {
      ...
      // if (range < 0.3) {
      //     ROS_WARN_STREAM("Small pt: range: " << range << ", axis: " <<
      //     axis);
      // }
      auto point = range * axis;
      pcl::PointXYZ pt;
...
void LivoxPointsPlugin::PublishPointCloud2XYZRTLT(
  ...
  for (int i = 0; i < points_pair.size(); ++i) {
    ...
    if (verticle_index < verticalRayCount && horizon_index < rayCount) {
      ...
      pt.timestamp =
          static_cast<double>(1e9 / 200000 * i) + header_timestamp_sec_nsec;
  ...
  for (int i = 0; i < points_pair.size(); ++i) {
    ...
    if (verticle_index < verticalRayCount && horizon_index < rayCount) {
      ...
      ray.Euler(ignition::math::Vector3d(0.0, rotate_info.zenith,
                                         rotate_info.azimuth));
      //                auto axis = rotate * ray * math::Vector3(1.0, 0.0, 0.0);
      //                auto point = range * axis + world_pose.Pos(); Convert to
